  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="sheet-content app-container fixed inset-x-0 bottom-0 z-50 flex max-h-[min(85dvh,100%)] flex-col rounded-t-[28px] bg-surface pb-[max(env(safe-area-inset-bottom),1rem)] shadow-[0_-8px_40px_rgb(38_83_180/0.14)] focus:outline-none">
          <div className="flex justify-center pt-3">
            <span className="h-1.5 w-10 rounded-full bg-border-soft" aria-hidden="true" />
          </div>
          <div className="flex items-start justify-between gap-4 page-px pb-2 pt-3">
          ...
          <div className="overflow-y-auto page-px pb-4 pt-2">{children}</div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )
